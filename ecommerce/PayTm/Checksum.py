...
def verify_checksum(param_dict, merchant_key, checksum):
    ...
    # Ensure both calculated_checksum and checksum are bytes
    calculated_checksum_bytes = calculated_checksum.encode('utf-8') if isinstance(calculated_checksum, str) else calculated_checksum

    # Compare calculated_checksum with provided checksum
    return calculated_checksum_bytes == checksum.encode('utf-8') if isinstance(checksum, str) else checksum
...
__unpad__ = lambda s: s[0:-s[-1] if isinstance(s[-1], int) else ord(s[-1])]
